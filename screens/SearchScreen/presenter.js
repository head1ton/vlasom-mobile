    ...
    <ScrollView refreshControl={<RefreshControl refreshing={props.isFetching} onRefresh={props.refresh} tintColor={'black'} />} >
        <View style={styles.container}>
            {props.search.length === 0 && props.searchingBy.length > 1 ? (
                <Text style={styles.notFound}>Nothing found for "{props.searchingBy}"</Text>
            ) : props.search.map(photo => <SquarePhoto key={photo.id} imageURL={photo.image} photoId={photo.id} />)}
        </View>
    </ScrollView>
)
